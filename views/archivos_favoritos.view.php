            ...
            <tr>
                <td class="py-2 px-4 border-b">
                    <a href="archivo.php?id=<?= $archivo['id'] ?>" class="text-blue-500 hover:underline">
                        <i class="fas fa-file-alt"></i> <?= htmlspecialchars($archivo['nombre_archivo']) ?>
                    </a>
                </td>
                ...
                <td class="py-2 px-4 border-b">
                    <a href="archivos_usuario.php?id=<?= $archivo['usuario_subio_id'] ?>" class="text-blue-500 hover:underline">
                        <i class="fas fa-user"></i> <?= htmlspecialchars($archivo['username']) ?> (<?= htmlspecialchars($archivo['nombre'] . ' ' . $archivo['apellidos']) ?>)
                    </a>
                </td>
                <td class="py-2 px-4 border-b"><i class="fas fa-file-archive"></i> <?= number_format($archivo['tamaño'] / 1024, 2) ?> KB</td>
                <td class="py-2 px-4 border-b">
                    <button onclick="toggleFavorite(<?= $archivo['id'] ?>)" class="bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600">
                        <i class="fas fa-heart-broken"></i> Quitar Favorito
                    </button>
                </td>
            </tr>
            <?php endforeach; ?>
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            .finally(() => {
                ...
                button.innerHTML = '<i class="fas fa-heart-broken"></i> Quitar Favorito';
            });
        }
    });
}
